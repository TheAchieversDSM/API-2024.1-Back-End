...
class ProductController {
    ...
    public async getAllRecomendation(req: Request, res: Response): Promise<void> {
        try {
            const productsUuid = req.params.productsUuid.split(',');
            console.log('productsUuid:', productsUuid);
            const recommendation = await productService.getAllRecomendationByComments(productsUuid.map((uuid) => parseInt(uuid)));
